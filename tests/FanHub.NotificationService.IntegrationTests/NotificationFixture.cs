...
public sealed class NotificationFixture : IAsyncLifetime
{
    ...
    private readonly string adminConnection = Environment.GetEnvironmentVariable("FANHUB_TEST_SQL_ADMIN")
        ?? throw new InvalidOperationException("Run docker/chinhduc/Test-NotificationService.ps1 to supply isolated test infrastructure settings.");
    ...
    private string Database => "fanhub_notification_test_" + suffix;
    private string Login => "notification_test_" + suffix;
    private string Vhost => "notification-test-" + suffix;
    ...
    public async Task InitializeAsync()
    {
        ...
        var root = FindRoot();
        var migrations = Directory.GetFiles(Path.Combine(root, "database/notification"), "*.sql")
        ...
        Bus = MassTransit.Bus.Factory.CreateUsingRabbitMq(config =>
        {
            ...
            config.ReceiveEndpoint("notification-test-observer", endpoint => endpoint.Handler<BookingChangedEvent>(context =>
            {
        ...
        factory = new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
        {
            ...
            builder.UseSetting("ConnectionStrings:NotificationDb", Connection);
            builder.UseSetting("Jwt:Secret", SigningKey);
            builder.UseSetting("Firebase:Enabled", "false");
            builder.ConfigureServices(services => services.AddSingleton<FanHub.NotificationService.Push.IPushSender>(Sender));
        ...
        await EventuallyAsync(async () => (await management.GetAsync($"/api/queues/{Vhost}/fanhub-notification-projections")).IsSuccessStatusCode);
    }
    ...
    public NotificationDbContext Db() => new(new DbContextOptionsBuilder<NotificationDbContext>().UseSqlServer(Connection).Options);
    ...
    public async Task WaitForFaultAsync()
    {
        await EventuallyAsync(async () =>
        {
            var response = await management.GetAsync($"/api/queues/{Vhost}/fanhub-notification-projections_error");
            if (!response.IsSuccessStatusCode) return false;
            var json = await response.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
            return json.TryGetProperty("messages", out var count) && count.GetInt32() > 0;
        });
    }
