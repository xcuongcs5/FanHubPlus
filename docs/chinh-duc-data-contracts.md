...
Token đăng ký một lần theo provider/device và provider/hash. Khi người dùng đăng nhập lại trên cùng máy, chuyển token sang user hiện tại trong transaction. Worker kiểm tra lại token vẫn thuộc recipient trước khi gửi delivery cũ; nếu đổi owner thì đánh dấu Skipped. Không ghi token nguyên văn vào log, vô hiệu hóa token khi provider báo invalid. NotificationService hiện có worker Firebase Admin SDK HTTP v1, lease/binding guard/retry; iOS đi qua FCM với APNs cấu hình trong Firebase. Chưa có raw APNs sender hoặc real-time Redis/WebSocket. Xem README Notification để cấu hình và hiểu giới hạn at-least-once.
