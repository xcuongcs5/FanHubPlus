...
try {
    ...
    & dotnet test (Join-Path $PSScriptRoot '../../tests/FanHub.BookingService.IntegrationTests/FanHub.BookingService.IntegrationTests.csproj') --nologo
    if ($LASTEXITCODE -ne 0) { throw 'Booking integration tests failed.' }
} finally {
