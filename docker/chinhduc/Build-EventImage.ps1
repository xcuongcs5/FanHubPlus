...
$buildRoot = Join-Path $temporaryRoot ('fanhub-event-build-' + [Guid]::NewGuid().ToString('N'))
try {
    ...
    foreach ($project in @('services/dotnet/FanHub.EventService', 'services/dotnet/shared/FanHub.Shared.Contracts')) {
    ...
    & docker build -t fanhub-chinhduc-event-service:local -f (Join-Path $buildRoot 'services/dotnet/FanHub.EventService/Dockerfile') $buildRoot
    if ($LASTEXITCODE -ne 0) { throw 'Event Docker image build failed.' }
} finally {
    ...
    if (-not $resolved.StartsWith($allowedRoot, [StringComparison]::OrdinalIgnoreCase) -or
        [IO.Path]::GetFileName($resolved) -notmatch '^fanhub-event-build-[a-f0-9]{32}$') {
