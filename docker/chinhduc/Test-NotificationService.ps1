...
try {
    ...
    & dotnet test (Join-Path $PSScriptRoot '../../tests/FanHub.NotificationService.IntegrationTests/FanHub.NotificationService.IntegrationTests.csproj') --nologo
    if ($LASTEXITCODE -ne 0) { throw 'Notification integration tests failed.' }
} finally {
