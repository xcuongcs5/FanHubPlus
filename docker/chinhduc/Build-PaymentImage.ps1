...
$buildRoot = Join-Path $temporaryRoot ('fanhub-payment-build-' + [Guid]::NewGuid().ToString('N'))
try {
    ...
    foreach ($project in @('services/dotnet/FanHub.PaymentWalletService', 'services/dotnet/shared/FanHub.Shared.Contracts', 'services/dotnet/shared/FanHub.Shared.Common')) {
    ...
    & docker build -t fanhub-chinhduc-payment-service:local -f (Join-Path $buildRoot 'services/dotnet/FanHub.PaymentWalletService/Dockerfile') $buildRoot
    if ($LASTEXITCODE -ne 0) { throw 'Payment Docker image build failed.' }
} finally {
    ...
    if (-not $resolved.StartsWith($allowedRoot, [StringComparison]::OrdinalIgnoreCase) -or
        [IO.Path]::GetFileName($resolved) -notmatch '^fanhub-payment-build-[a-f0-9]{32}$') {
