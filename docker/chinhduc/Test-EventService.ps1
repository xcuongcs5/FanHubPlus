...
try {
    ...
    & dotnet test (Join-Path $PSScriptRoot '../../tests/FanHub.EventService.IntegrationTests/FanHub.EventService.IntegrationTests.csproj') --nologo
    if ($LASTEXITCODE -ne 0) { throw 'Event integration tests failed.' }
} finally {
